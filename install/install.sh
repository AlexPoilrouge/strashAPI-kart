...
if [ -n "${TEMPLATE_DEST_STRASH_API_SERVICE}" ] && [ -f "${TEMPLATE_DEST_STRASH_API_SERVICE}" ]; then
    which systemctl &> /dev/null && systemctl enable --now "$( basename "${TEMPLATE_DEST_STRASH_API_SERVICE}" )"
fi

if [ -n "${TEMPLATE_DEST_STRASH_API}" ] && [ -f "${TEMPLATE_DEST_STRASH_API}" ]; then
